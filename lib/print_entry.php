...
function replace_entry($fetchData)
{
    $html_txt = preg_replace(
        '/<\/h2>/',
        '</h2><div class="main-text-area">',
        $fetchData["html"]
    );
    $html_txt = preg_replace(
        '/(<h2 class="section">)/',
        '$1<i class="fa fa-angle-down fa-1x"></i>',
        $html_txt,
        1
    );
    $html_txt = preg_replace(
        '/(\n.*)(<h2 class="section">)/',
        '</div>$2<i class="fa fa-angle-down fa-1x"></i>',
        $html_txt
    );
...
{
    // http://www.webcyou.com/?p=964
    //画像タグ置換よけ
    $value = mb_ereg_replace(
        '(<img src=")(http|https|ftp)://([[a-zA-Z0-9]\+\$\;\?\.%,!#~*/:@&=_-]+)">',
        '[image](\\3)',
        $value
    );
    //URLアンカー化
...
<ul class="breadcrumb-list">  <!-- パンくずリスト -->
  <?php if ($fetchData["category"] != "none") {
...
<?php if ($fetchData["eyecatch"] === "none") {
    $fetchData["eyecatch"] = "/resource/img/eyecatch-none.png";
} ?>
<div class="eyecatch" style="background-image:url('<?php echo $host .
    $fetchData["eyecatch"]; ?>')"></div>
