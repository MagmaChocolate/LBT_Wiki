...
$(function(){
  ...
  for(var i = 0,h2count = $('div.main-text-area > h2').length;i < h2count;i++){
    listCount[i] = $('div.main-text').eq(i).find('a').length;
  }
  var html = '',inhtml = '';
  for (var j in listCount){
    html = $('div.main-text').eq(j).html();
    inhtml = '<div class="contents-count">記事数：'+listCount[j]+'件</div>';
    $('div.main-text').eq(j).html(inhtml + html);
  }
});
